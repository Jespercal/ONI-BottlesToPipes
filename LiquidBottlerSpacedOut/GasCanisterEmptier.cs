...
namespace Alesseon.LiquidBottler.Building
{
    ...
    class GasCanisterEmptier : StateMachineComponent<GasCanisterEmptier.StatesInstance>, IGameObjectEffectDescriptor
    {
        ...
        private static readonly EventSystem.IntraObjectHandler<GasCanisterEmptier> OnRefreshUserMenuDelegate = new EventSystem.IntraObjectHandler<GasCanisterEmptier>((component, data) => component.OnRefreshUserMenu(data));
        private static readonly EventSystem.IntraObjectHandler<GasCanisterEmptier> OnCopySettingsDelegate = new EventSystem.IntraObjectHandler<GasCanisterEmptier>((component, data) => component.OnCopySettings(data));
        ...
        {
            Game.Instance.userMenu.AddButton(gameObject, allowManualPumpingStationFetching ? new KIconButtonMenu.ButtonInfo("action_empty_contents", UI.USERMENUACTIONS.MANUAL_PUMP_DELIVERY.DENIED_GAS.NAME, new System.Action(OnChangeAllowManualPumpingStationFetching), tooltipText: (UI.USERMENUACTIONS.MANUAL_PUMP_DELIVERY.DENIED_GAS.TOOLTIP)) : new KIconButtonMenu.ButtonInfo("action_empty_contents", UI.USERMENUACTIONS.MANUAL_PUMP_DELIVERY.ALLOWED_GAS.NAME, new System.Action(OnChangeAllowManualPumpingStationFetching), tooltipText: (UI.USERMENUACTIONS.MANUAL_PUMP_DELIVERY.ALLOWED_GAS.TOOLTIP)), 0.4f);
        }

        private void OnCopySettings(object data)
        {
            allowManualPumpingStationFetching = ((GameObject)data).GetComponent<GasCanisterEmptier>().allowManualPumpingStationFetching;
            smi.RefreshChore();
        }

        public class StatesInstance :
          GameStateMachine<States, StatesInstance, GasCanisterEmptier, object>.GameInstance
        {
            private FetchChore chore;

            public MeterController meter { get; private set; }

            public StatesInstance(GasCanisterEmptier smi)
              : base(smi)
            {
            ...
            public void CreateChore()
            {
                ...
                else
                {
                    ...
                    if (!master.allowManualPumpingStationFetching)
                        forbidden_tags = new Tag[1] {GameTags.GasSource};
                    else
        ...
        public class States :
          GameStateMachine<GasCanisterEmptier.States, GasCanisterEmptier.StatesInstance, GasCanisterEmptier>
        {
            ...
            public override void InitializeStates(out BaseState default_state)
            {
                ...
                statusItem = new StatusItem(nameof(GasCanisterEmptier), "", "", "", StatusItem.IconType.Info, NotificationType.Neutral, false, OverlayModes.None.ID);
                statusItem.resolveStringCallback = ((str, data) =>
                {
                    GasCanisterEmptier bottleEmptier = (GasCanisterEmptier)data;
                    if (bottleEmptier == null)
                        return str;
                    return bottleEmptier.allowManualPumpingStationFetching ? BUILDING.STATUSITEMS.CANISTER_EMPTIER.ALLOWED.NAME : BUILDING.STATUSITEMS.CANISTER_EMPTIER.DENIED.NAME;
                });
                statusItem.resolveTooltipCallback = ((str, data) =>
                {
                    ...
                    if (bottleEmptier == null)
                        return str;
                    return bottleEmptier.allowManualPumpingStationFetching ? BUILDING.STATUSITEMS.CANISTER_EMPTIER.ALLOWED.TOOLTIP : BUILDING.STATUSITEMS.CANISTER_EMPTIER.DENIED.TOOLTIP;
                });
